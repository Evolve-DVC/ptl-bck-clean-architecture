...
/**
 * Plantilla base para comandos de actualización en dominio.
 *
 * @param <C> tipo de contexto de entrada y salida
 * @param <K> tipo de identificador de la entidad
 */
public abstract class UpdateCommandAbstract<C, K> extends CommandProcessAbstract<C, C> {
    private final IGenericService<C, K> service;

    /**
     * Crea la base del comando de actualización.
     *
     * @param service  servicio de persistencia del dominio
     * @param executor ejecutor para el flujo de procesamiento
     */
    protected UpdateCommandAbstract(IGenericService<C, K> service, Executor executor) {
        super(executor);
